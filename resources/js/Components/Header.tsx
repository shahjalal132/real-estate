...
export default function Header() {
    ...
    const renderNavItem = (
            ...
            >
        {
            isMegaMenu?(
                    <button
                        type = "button"
                        className = {`relative group tracking-[0.5px] font-normal transition-all duration-200 whitespace-nowrap text-xs md:text-sm capitalize cursor-pointer px-1 py-1.5 rounded-md ${activeMenu === item.megaMenuId
                    ...
                    { item.label }
        {
            item.hasDropdown && (
                <ChevronDown
                                    className={
                `h-3 w-3 shrink-0 transition-transform duration-200 ${activeMenu === item.megaMenuId
    ...
    <Link
                        href= { item.link }
className = {`relative group tracking-[0.5px] font-normal transition-all duration-200 whitespace-nowrap text-xs md:text-sm capitalize px-1 py-1.5 rounded-md ${activeMenu === item.megaMenuId
    ? "text-[#0066cc] bg-[#F0F7FF]"
    : "text-[#4A4A4A] hover:text-[#0066cc] hover:bg-[#F0F7FF]/50"
    }`}
                    >
                ...
                )}
{
    ...
    onMouseEnter = {() => {
        ...
        setActiveMenu(item.megaMenuId || null);
                        ...
                        />
    < div
className = {`absolute top-full pt-4 z-[100] ${shouldRightAlign || isRightNav
    ? "right-0"
    : "left-0"
    }`}
onMouseEnter = {() => {
...
onMouseLeave = {() => {
    ...
    <button
                                        className={
    `relative group tracking-[0.5px] font-normal transition-all duration-200 whitespace-nowrap flex items-center gap-1.5 md:gap-2 text-xs md:text-sm capitalize px-1 py-1.5 rounded-md ${moreMenuOpen
        ? "text-[#0066cc] bg-[#F0F7FF]"
        : "text-[#4A4A4A] hover:text-[#0066cc] hover:bg-[#F0F7FF]/50"
        }`
}
...
More
    < ChevronDown
className = {`h-3 w-3 shrink-0 transition-transform duration-200 ${moreMenuOpen
    ? "rotate-180 text-[#0066cc]"
    : "text-[#4a4a4a]"
    }`}
                                        />
                                                    ...
                                                    >
{
    item.type ===
        ...
        item.hasDropdown ? (
            <button
                                                                type= "button"
                                                                className={`block w-full text-left px-4 py-2 text-sm tracking-[0.5px] transition-colors capitalize ${activeMenu ===
        item.megaMenuId
        ? "text-[#0066cc] bg-[#F0F7FF]"
        ...
        { item.label }
{
    item.hasDropdown && (
        <ChevronDown
                                                                            className={
        `h-3 w-3 transition-all duration-300 ${activeMenu ===
            item.megaMenuId
            ? "rotate-180 text-[#0066cc]"
            : "text-[#4a4a4a]"
            }`
    ...
    <Link
                                                                href= { item.link }
className = {`block px-4 py-2 text-sm tracking-[0.5px] transition-colors capitalize ${activeMenu ===
    item.megaMenuId
    ? "text-[#0066cc] bg-[#F0F7FF]"
    : "text-[#4A4A4A] hover:bg-gray-50"
    }`}
