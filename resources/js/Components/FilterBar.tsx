...
export default function FilterBar({
    ...
    const [filtersExpanded, setFiltersExpanded] = useState(showExpandedOnly);
    ...
    const selectedAuction =
        AUCTION_OPTIONS.find((opt) => opt.value === auctionValue)?.label ||
        "Auctions";

    const selectedRegion =
        REGION_OPTIONS.find((opt) => opt.value === region)?.label || "All";
    const selectedAuctionEvent =
        AUCTION_EVENT_OPTIONS.find((opt) => opt.value === auctionEvent)
            ?.label || "All";
    ...
    // If showExpandedOnly is true, skip the button row and show only expanded filters
    if (showExpandedOnly) {
        return (
