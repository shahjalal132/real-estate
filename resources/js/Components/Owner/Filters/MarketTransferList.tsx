...
export default function MarketTransferList({
    ...
    const [availableMarkets, setAvailableMarkets] =
        useState<string[]>(SAMPLE_MARKETS);
    ...
    const [selectedPropertyType, setSelectedPropertyType] = useState("Office");
    const [showSubcontinentDropdown, setShowSubcontinentDropdown] =
        useState(false);
    const [showCountryDropdown, setShowCountryDropdown] = useState(false);
    const [showPropertyTypeDropdown, setShowPropertyTypeDropdown] =
        useState(false);
    const subcontinentDropdownRef = useRef<HTMLDivElement>(null);
    ...
    useEffect(() => {
        ...
        setAvailableMarkets(
            SAMPLE_MARKETS.filter((market) => !selectedMarkets.includes(market))
        );
    }, [selectedMarkets]);
    ...
    const handleMoveRight = () => {
        if (highlightedAvailable) {
            ...
            setAvailableMarkets(
                availableMarkets.filter((m) => m !== highlightedAvailable)
            );
    ...
    const handleMoveAllRight = () => {
        ...
        setSelected(newSelected);
        setAvailableMarkets(
            availableMarkets.filter((m) => !filteredAvailable.includes(m))
        );
        setHighlightedAvailable(null);
        onChange?.(newSelected);
    };
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (
                subcontinentDropdownRef.current &&
                !subcontinentDropdownRef.current.contains(event.target as Node)
            ) {
                setShowSubcontinentDropdown(false);
            }
            if (
    ...
    return (
        <div className="space-y-4">
            {/* Subcontinents Filter */}
            <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                    Subcontinents
                </label>
                <div className="relative" ref={subcontinentDropdownRef}>
                    <input
                        type="text"
                        ...
                        onFocus={() => setShowSubcontinentDropdown(true)}
                        placeholder="Select"
                            ...
                            {SUBCONTINENTS.map((subcontinent) => (
                                <button
                                    ...
                                    className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${
                                        selectedSubcontinent === subcontinent
            ...
            {/* Country and Property Type Filters - Two Columns */}
                ...
                <div>
                    ...
                    <div className="relative" ref={propertyTypeDropdownRef}>
                        <input
                            ...
                            onFocus={() => setShowPropertyTypeDropdown(true)}
                            placeholder="Select"
                            readOnly
                            className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 pr-10 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                            ...
                            <ul className="divide-y divide-gray-200">
                                {filteredAvailable.map((market) => (
                                    <li
                                        ...
                                        onDoubleClick={() =>
                                            handleDoubleClickAvailable(market)
                                        }
                    ...
                    <div className="flex-1 border border-gray-300 rounded-md bg-white min-h-[300px] max-h-[400px] overflow-y-auto">
                        {selected.length > 0 ? (
                            ...
                            <div className="p-4 text-center text-sm text-gray-400 space-y-1">
                                <p>
                                    Select Markets from the list on the left by
                                    double clicking.
                                </p>
                                <p>Or</p>
                                <p>
