import { Building, Globe, Linkedin, Mail, MapPin, Phone, Trophy, User } from "lucide-react";
...
<div className="flex flex-col gap-1 min-w-0 flex-1" >
    <h3 className="text-base font-bold text-gray-900 leading-tight" > { broker.name } </h3>
        < p className = "text-xs text-gray-600 font-medium" > { broker.title } </p>
            < p className = "text-xs text-gray-800 font-semibold mt-1" > { broker.company } </p>

{/* Address */ }
<div className="text-xs text-gray-600 mt-1" >
    { broker.address && <p>{ broker.address } </p> }
    <p>
{ [broker.city, broker.state, broker.postal_code].filter(Boolean).join(", ") }
</p>
    ...
    broker.email && (
        <div className="flex items-center gap-2 text-xs text-blue-600 hover:text-blue-800" >
            <Mail className="h-3 w-3 shrink-0" />
                <a href={ `mailto:${broker.email}` } className = "hover:underline truncate" title = { broker.email } >
                    { broker.email }
                    </a>
                    </div>
                    )
    ...
    broker.linkedin && (
        <div className="flex items-center gap-2 text-xs text-blue-600 hover:text-blue-800" >
            <Linkedin className="h-3 w-3 shrink-0" />
                <a href={ broker.linkedin.startsWith('http') ? broker.linkedin : `https://${broker.linkedin}` } target = "_blank" rel = "noopener noreferrer" className = "hover:underline truncate" >
                    { broker.name }
                    </a>
                    </div>
                     )
    ...
    broker.website && (
        <div className="flex items-center gap-2 text-xs text-blue-600 hover:text-blue-800" >
            <Globe className="h-3 w-3 shrink-0" />
                <a href={ broker.website.startsWith('http') ? broker.website : `https://${broker.website}` } target = "_blank" rel = "noopener noreferrer" className = "hover:underline truncate" >
                    { broker.company }
                    </a>
                    </div>
                     )
