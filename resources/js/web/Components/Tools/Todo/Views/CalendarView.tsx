...
import { ChevronLeft, ChevronRight, Plus, ChevronDown, Filter, Settings2, Search, Calendar } from "lucide-react";
...
export default function CalendarView({ tasks, onAddTask }: CalendarViewProps) {
    ...
    <div className="flex items-center gap-3" >
        <button onClick={ handlePrevWeek } className = "p-1 hover:bg-gray-100 rounded text-gray-500" >
            <ChevronLeft size={ 18 } />
                </button>
                < button onClick = { handleToday } className = "text-sm font-medium text-gray-700 hover:text-black" >
                    Today
                    </button>
                    < button onClick = { handleNextWeek } className = "p-1 hover:bg-gray-100 rounded text-gray-500" >
                        <ChevronRight size={ 18 } />
                            </button>
                            </div>

                            < h2 className = "text-lg font-medium text-gray-800 ml-1" > { currentMonthYear } </h2>
                                </div>
    ...
    <div className="flex-1 grid grid-cols-7 h-full overflow-hidden divide-x divide-gray-200" >
    {
        weekDays.map((date, idx) => {
            ...
            return (
                <div key= { idx } className = "flex flex-col h-full min-w-0 bg-white group" >
                    {/* Column Header */ }
        ...
        { date.toLocaleDateString("en-US", { weekday: "short" }).toUpperCase() }
            ...
            < div className = {`text-2xl font-normal leading-none mt-1 w-9 h-9 flex items-center justify-center rounded-md ${isCurrentDay ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-800'
                }`} >
        { date.getDate() }
        </div>
        </div>
    ...
    <div className="flex-1 p-2 space-y-2 overflow-y-auto" >
    {
        dayTasks.map(task => (
            <div 
                                        key= { task.id } 
                                        className = {`p-3 rounded border text-sm shadow-sm cursor-pointer hover:shadow-md transition-all ${task.completed
