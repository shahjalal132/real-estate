...
export default function Calendar() {
    ...
    const renderMonthGrid = () => {
        ...
        for (let i = 0; i < firstDay; i++) {
            days.push(<div key={`empty-${i}`} className = "min-h-[120px] bg-gray-50/50 border-r border-b border-gray-100" > </div>);
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        days.push(
            <div key={ day } className = {`min-h-[120px] bg-white border-r border-b border-gray-200 p-2 hover:bg-gray-50 transition-colors group relative ${isToday ? 'bg-blue-50/30' : ''}`}>
                    ...
                    < nav className = "flex-1 overflow-y-auto px-2 space-y-0.5 mt-2" >
                        <div className="px-3 pb-2 text-xs font-semibold text-gray-500 uppercase tracking-wider" >
                            My Calendars
                                ...
                                < SidebarItem icon = {< div className = "w-3 h-3 rounded bg-blue-500" />} label = "My Schedule" active />
                                    <SidebarItem icon={ <div className="w-3 h-3 rounded bg-green-500" />} label = "Open Houses" />
                                        <SidebarItem icon={ <div className="w-3 h-3 rounded bg-red-500" />} label = "Deadlines" />
                                            <SidebarItem icon={ <div className="w-3 h-3 rounded bg-purple-500" />} label = "Holidays" />
                                                ...
                                                <div className="pt-6 pb-2 px-3 flex items-center justify-between text-xs font-semibold text-gray-500 uppercase tracking-wider group cursor-pointer" >
                                                    <span>Team Calendars </span>
                                                        < Plus size = { 14} className = "opacity-0 group-hover:opacity-100 transition-opacity" />
                                                            ...
                                                            < SidebarItem icon = {< Users size = { 16} />} label = "Sales Team" />
                                                                <SidebarItem icon={
                                                                    <Users size={ 16 } />} label="Marketing" / >
                                                                        </nav>

    {/* Mini Calendar in Sidebar */ }
    <div className="p-4 border-t border-gray-200" >
        <div className="text-sm font-semibold mb-2" > August 2025 </div>
            < div className = "grid grid-cols-7 gap-1 text-center text-[10px] text-gray-500" >
                { ['S', 'M', 'T', 'W', 'T', 'F', 'S'].map(d => <div key={ d } > { d } </div>) }
    {
        Array.from({ length: 31 }, (_, i) => (
            <div key= { i } className = {`h-5 w-5 flex items-center justify-center rounded-full ${i + 1 === 4 ? 'bg-blue-600 text-white' : 'hover:bg-gray-100'}`}>
                { i+ 1
}
</div>
                            ))}
...
<main className="flex-1 flex flex-col min-w-0 bg-white" >
    ...
    < div className = "flex items-center bg-gray-100 rounded-md p-0.5 ml-4" >
        <button onClick={ prevMonth } className = "p-1 hover:bg-white hover:shadow-sm rounded transition-all" >
            <ChevronLeft size={ 18 } className = "text-gray-600" />
                </button>
                < button onClick = {() => setCurrentDate(new Date())} className = "px-3 py-1 text-sm font-medium text-gray-700 hover:bg-white hover:shadow-sm rounded transition-all" >
                    Today
                    </button>
                    < button onClick = { nextMonth } className = "p-1 hover:bg-white hover:shadow-sm rounded transition-all" >
...
Everyone
    ...
    < div className = "flex items-center gap-2 text-sm text-gray-500" >
        <span className="flex items-center gap-1" > <div className="w-2 h-2 rounded-full bg-green-500" > </div> Open House</span >
            <span className="flex items-center gap-1" > <div className="w-2 h-2 rounded-full bg-blue-500" > </div> Viewing</span >
                <span className="flex items-center gap-1" > <div className="w-2 h-2 rounded-full bg-red-500" > </div> Closing</span >
                    </div>
                    </div>
...
<div className="flex-1 overflow-auto bg-white" >
    ...
    {
        DAYS.map(day => (
            <div key= { day } className = "py-2 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider bg-gray-50" >
            { day }
            </div>
        ))
    }
...
function SidebarItem({ icon, label, active = false, hasSubmenu = false }: any) {
    return (
        <div
            className= {`flex items-center justify-between px-3 py-2 rounded-md cursor-pointer group text-sm ${active ? "bg-blue-50 text-blue-600" : "text-gray-600 hover:bg-gray-100"
            }`
}
        >
    <div className="flex items-center gap-3" >
...
{
    hasSubmenu && (
        <div className="text-gray-400 opacity-0 group-hover:opacity-100" >→</div>
            )
}
</div>
    );
